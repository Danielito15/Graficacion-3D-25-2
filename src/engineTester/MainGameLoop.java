...
public class MainGameLoop {
	...
	public static void main(String[] args) throws Exception {
        ...
        // --- Archivos MP3 de entrada y nombres de salida WAV ---
        Map<String, String> audios = Map.of(
            "agua", "agua.mp3",
            "LuisMi", "LuisMi.mp3"
        );

        // --- Parámetros personalizados para cada sonido: rolloff, referenceDistance, maxDistance ---
        Map<String, float[]> parametrosAudio = Map.of(
            "agua",    new float[]{1f, 0.2f, 0.5f},
            
            "LuisMi",  new float[]{2f, 1.5f, 5f}
        );
        ...
        // --- Conversión y carga ---
        Map<String, Source> fuentes = new HashMap<>();

        for (Map.Entry<String, String> entrada : audios.entrySet()) {
            String nombre = entrada.getKey();
            String archivoMP3 = entrada.getValue();

            String rutaMP3 = Paths.get("res", "Audios", archivoMP3).toString();
            String nombreWAV = nombre + ".wav";

            AudioConverter.convertToMono(rutaMP3, nombreWAV);

            // Cargar WAV a buffer
            String rutaRelativa = AudioConverter.extraerRutaRelativa("Audios/" + nombreWAV);
            int buffer = AudioMaster.loadSound(rutaRelativa);

            // Crear y configurar fuente
            float[] p = parametrosAudio.get(nombre);
            Source fuente = new Source(p[0], p[1], p[2]);
            fuente.setLooping(true);
            fuente.play(buffer);

            fuentes.put(nombre, fuente);
        }
        
        fuentes.get("LuisMi").setVolume(0.3f);
		...
		while (!Display.isCloseRequested()) {
			...
			fuentes.get("agua").setPosition(eartPos.x, eartPos.y, eartPos.z);
		...
		for (Source fuente : fuentes.values()) {
		    fuente.delate();
		}
